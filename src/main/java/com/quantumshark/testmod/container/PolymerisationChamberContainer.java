...
public class PolymerisationChamberContainer extends MachineContainerBase<PolymerisationChamberTileEntity> {

	public FunctionalIntReferenceHolder currentSmeltTime;
	
	// Server Constructor
	public PolymerisationChamberContainer(final int windowID, final PlayerInventory playerInv,
			final PolymerisationChamberTileEntity tile) {
		super(RegistryHandler.POLYMERISATION_CHAMBER_CONTAINER.get(), windowID, playerInv, tile);
		...
		// catalyst slot
		this.addSlot(new SlotItemHandler(tile.getInventory(), 1, 44, 35));

		// output slot
		this.addSlot(new SlotItemHandler(tile.getInventory(), 2, 126, 35));	// output
	...
	// Client Constructor
	public PolymerisationChamberContainer(final int windowID, final PlayerInventory playerInv, final PacketBuffer data) {
		this(windowID, playerInv, getTileEntity(playerInv, data));
	}

	private static PolymerisationChamberTileEntity getTileEntity(final PlayerInventory playerInv, final PacketBuffer data) {
		Objects.requireNonNull(playerInv, "playerInv cannot be null");
		Objects.requireNonNull(data, "data cannot be null");
		final TileEntity tileAtPos = playerInv.player.world.getTileEntity(data.readBlockPos());
		if (tileAtPos instanceof PolymerisationChamberTileEntity) {
			return (PolymerisationChamberTileEntity) tileAtPos;
		}

...
public class FractionalDistillationChamberContainer extends MachineContainerBase<FractionalDistillationChamberTileEntity> {

	public FunctionalIntReferenceHolder currentSmeltTime;
	
	// Server Constructor
	public FractionalDistillationChamberContainer(final int windowID, final PlayerInventory playerInv,
			final FractionalDistillationChamberTileEntity tile) {
		super(RegistryHandler.FRACTIONAL_DISTILLATION_CHAMBER_CONTAINER.get(), windowID, playerInv, tile);
		...
		// tanks (5)
		screenWidgets.add(new TankWidget(0, 22, 29, 18, 24));
		screenWidgets.add(new TankWidget(0, 44, 29, 18, 24));

		screenWidgets.add(new TankWidget(2, 103, 29, 18, 24));
		screenWidgets.add(new TankWidget(3, 125, 29, 18, 24));
		screenWidgets.add(new TankWidget(4, 147, 29, 18, 24));
	...
	// Client Constructor
	public FractionalDistillationChamberContainer(final int windowID, final PlayerInventory playerInv, final PacketBuffer data) {
		this(windowID, playerInv, getTileEntity(playerInv, data));
	}

	private static FractionalDistillationChamberTileEntity getTileEntity(final PlayerInventory playerInv, final PacketBuffer data) {
		Objects.requireNonNull(playerInv, "playerInv cannot be null");
		Objects.requireNonNull(data, "data cannot be null");
		final TileEntity tileAtPos = playerInv.player.world.getTileEntity(data.readBlockPos());
		if (tileAtPos instanceof FractionalDistillationChamberTileEntity) {
			return (FractionalDistillationChamberTileEntity) tileAtPos;
		}

...
public class FractionalDistillationChamberTileEntity extends MachineTileEntitySingleRecipeTypeBase {
	public double currentSmeltTime;
	public int maxSmeltTime = 100;

	public FractionalDistillationChamberTileEntity() {
		super(RegistryHandler.FRACTIONAL_DISTILLATION_CHAMBER_TILE_ENTITY.get());
	}

	@Override
	public Container createMenu(final int windowID, final PlayerInventory playerInv, final PlayerEntity playerIn) {
		return new FractionalDistillationChamberContainer(windowID, playerInv, this);
	}
	...
	public void tick() {
		...
		if (getRedstoneRunnable()) {
			...
			RecipeAndWrapper<FractionalDistillationChamberRecipe> match = this.findMatchingRecipe();
			if (match == null) {
				// reset progress if you remove the input item.
				this.currentSmeltTime = 0;
			} else {
				FractionalDistillationChamberRecipe recipe = match.recipe;
		...
		dirty |= AttemptFillBucket(0, 1, 2);
		dirty |= AttemptEmptyBucket(0, 1, 2);
		dirty |= AttemptFillBucket(1, 2, 3);
		dirty |= AttemptFillBucket(1, 3, 3);
		dirty |= AttemptFillBucket(1, 4, 3);
	...
	protected ITextComponent getDefaultName() {
		return new TranslationTextComponent("container." + TestMod.MOD_ID + ".fractional_distillation_chamber");
	}
	...
	protected IRecipeType<MachineRecipeBase> getRecipeType() {
		return RecipeInit.FRACTIONAL_DISTILLATION_CHAMBER_RECIPE_TYPE;
	}

	// the recipe template (combination of inputs and secondary outputs if any)
	@Override
	public RecipeTemplate getRecipeTemplate() {
		return RecipeTemplate.FRACTIONAL_DISTILLATION_CHAMBER;
	}
	...
	public boolean isItemValid(int slot, ItemStack stack) {
		if (slot == 0) {
			return isItemValidForFluidSlot(slot, 0, stack, true, true)
					|| isItemValidForFluidSlot(slot, 1, stack, true, true);
		} else if (slot == 1) {
			return isItemValidForFluidSlot(slot, 2, stack, false, true)
					|| isItemValidForFluidSlot(slot, 3, stack, false, true)
					|| isItemValidForFluidSlot(slot, 4, stack, false, true);
		} else {

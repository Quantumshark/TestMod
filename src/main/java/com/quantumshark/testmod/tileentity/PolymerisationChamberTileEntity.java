...
public class PolymerisationChamberTileEntity extends MachineTileEntitySingleRecipeTypeBase {
	public double currentSmeltTime;
	public int maxSmeltTime = 100;

	public PolymerisationChamberTileEntity() {
		super(RegistryHandler.POLYMERISATION_CHAMBER_TILE_ENTITY.get());
	}

	@Override
	public Container createMenu(final int windowID, final PlayerInventory playerInv, final PlayerEntity playerIn) {
		return new PolymerisationChamberContainer(windowID, playerInv, this);
	}
	...
	public void tick() {
		...
		if (getRedstoneRunnable()) {
			...
			RecipeAndWrapper<PolymerisationChamberRecipe> match = this.findMatchingRecipe();
			if (match == null) {
				// reset progress if you remove the input item.
				this.currentSmeltTime = 0;
			} else {
				PolymerisationChamberRecipe recipe = match.recipe;
	...
	protected ITextComponent getDefaultName() {
		return new TranslationTextComponent("container." + TestMod.MOD_ID + ".polymerisation_chamber");
	}

	@Override
	protected IRecipeType<MachineRecipeBase> getRecipeType() {
		return RecipeInit.POLYMERISATION_CHAMBER_RECIPE_TYPE;
	}

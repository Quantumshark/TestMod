...
public class CatalyticCrackingChamberTileEntity extends MachineTileEntitySingleRecipeTypeBase {
	public double currentSmeltTime;
	public int maxSmeltTime = 100;

	public CatalyticCrackingChamberTileEntity() {
		super(RegistryHandler.CATALYTIC_CRACKING_CHAMBER_TILE_ENTITY.get());
	}

	@Override
	public Container createMenu(final int windowID, final PlayerInventory playerInv, final PlayerEntity playerIn) {
		return new CatalyticCrackingChamberContainer(windowID, playerInv, this);
	}
	...
	public void tick() {
		...
		if (getRedstoneRunnable()) {
			...
			RecipeAndWrapper<CatalyticCrackingChamberRecipe> match = this.findMatchingRecipe();
			if (match == null) {
				// reset progress if you remove the input item.
				this.currentSmeltTime = 0;
			} else {
				CatalyticCrackingChamberRecipe recipe = match.recipe;
		...
		dirty |= AttemptFillBucket(0, 0, 3);
		dirty |= AttemptEmptyBucket(0, 0, 3);
		dirty |= AttemptFillBucket(1, 1, 4);
	...
	protected ITextComponent getDefaultName() {
		return new TranslationTextComponent("container." + TestMod.MOD_ID + ".catalytic_cracking_chamber");
	}
